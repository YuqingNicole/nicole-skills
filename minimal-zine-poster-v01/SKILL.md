---
name: minimal-zine-poster-v01
description: Generate Minimal Zine Poster v0.1 poetic paper-poster prompts and matching generated images. Also respond to aliases gc-minimal-zine-poster-v0-1, minimal-zine-poster-v0.1, and minimal-zine-poster-v01. Use when the user gives a theme, sentence, object, mood, article idea, photo, or content brief and wants a quiet Japanese/Korean zine-like editorial poster with large negative space, aged paper texture, experimental typography, restrained color accents, and a generated bitmap image.
---
...
3. Select a variation recipe.
   ...
   - Match the recipe to the theme when possible:
     - sky, memory, distance, architecture, or dreams: prefer `upper-middle`/`center-fragment`, old printed illustration or torn-paper clipping, archive microtext, cobalt or ultramarine.
     - object, food, plant, book, tool, or keepsake: prefer `single-specimen`, object specimen or flat silhouette, almost-textless caption, one saturated printed subject.
     - sentence, slogan, title, or poem: prefer `type-led`, headline-as-object or text inside a color block, with image secondary or absent.
     - sea, summer, travel, weather, or place: prefer `lower-left-float` or `dual-panel`, tiny faded photo or abstract texture window, date/weather microtext.
...
4. Write the final image prompt.
   ...
   - If exact text accuracy is important, generate a text-light or text-free poster first, then add the final title with a deterministic local layout or a precise image edit. Do not rely on the image model for long, strict, or brand-critical text.
   - For CJK titles, prefer one short phrase of 2-6 characters. If the model spaces characters apart or distorts glyphs, regenerate once with `no title text, only tiny illegible archive microtext`, then add the title outside the generation step.
...
6. Return according to the active image tool constraints.
   - Before generation, it is fine to show the final prompt when that helps the user understand the direction.
   - After generation, if the image tool requires silence, show only the image. Otherwise include the final prompt and recipe.
...
Use this full format only when the active image tool allows commentary after generation. If the image tool requires no post-generation text, show the final prompt before generation and then return only the generated image.
...
If the first result misses the intended style, regenerate once with a targeted fix:

- Too little negative space: shrink the visual cluster to 10%-16%, move it away from the edges, and explicitly keep 80%-90% plain aged paper.
- Accent color too weak: make the anchor `opaque fully saturated cobalt-blue` or another vivid single ink area occupying 20%-35% of the cluster; avoid pale, faded, pastel, or muted wording for the anchor.
- Too commercial or poster-like: remove large headline hierarchy, reduce contrast, replace clean layout wording with scanned paper, xerox softness, letterpress bleed, and drifting microtext.
- Too literal or busy: reduce the subject to one specimen, clipping, silhouette, old printed illustration, or small relation between two elements.
- Text is wrong: remove readable text from the generated image and keep only microtext or abstract letter fragments; add exact text later with deterministic typography.

## Example Requests

- "用 $minimal-zine-poster-v01 做一张关于雨天的图"
- "用 $minimal-zine-poster-v01 标准模式，做一张关于旧书的图"
- "用 $gc-minimal-zine-poster-v0-1 做一张关于天空之城的海报"
- "用这张照片做一张同风格 poster"
